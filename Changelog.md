...
- Updaetd Clover to r4674
- Updated ACPIBatteryManager to 1.81.4
- Removed Shiki since it is replaced by WhateverGreen
- Updated README and attach for Changelog to make a more clean output style
- Mojave preparation
...
- Confirmed working flawslessly after updating from ```10.13.0``` to ```10.13.3```, note: for non-iris version working ```clipboard```, you need to re-run deploy to rebuild the ```dyld_shared_cache```. Patch remain the same.
